...
}: { lat: number; long: number; visible: boolean; color?: string; timestamp: number; isUserLocation?: boolean }) {
  ...
  return (
    <>
      ...
      <mesh position={[position.x, position.y, position.z]} scale={scale}>
        <sphereGeometry args={[0.008, 16, 16]} />
        <meshBasicMaterial color={color} transparent opacity={uniformOpacity} toneMapped={false} />
      </mesh>
    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-[#0d1b2a] via-[#1b263b] to-[#0d1b2a] overflow-hidden">
      ...
      <div className="absolute bottom-6 left-6 z-10">
        <Button
          ...
          className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-semibold px-6 py-2 rounded-lg shadow-lg hover:shadow-cyan-500/50 transition-all duration-300 flex items-center gap-2"
        >
      ...
      {locationPermission === "pending" && (
        <div className="absolute top-6 left-6 z-10">
          <Button
            ...
            className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold px-4 py-2 rounded-lg shadow-lg hover:shadow-purple-500/50 transition-all duration-300 flex items-center gap-2"
          >
      ...
      {showTransactions && (
        <div className="absolute bottom-24 left-6 z-10 w-96">
          <Card className="bg-black/90 border-cyan-500/50 backdrop-blur-md shadow-2xl shadow-cyan-500/20 rounded-xl overflow-hidden">
            <div className="p-6">
              ...
              <div className="max-h-96 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-black/80 border-b border-cyan-500/30">
                    <tr>
                      <th className="text-left py-2 px-2 text-cyan-400 font-semibold">From</th>
                      <th className="text-left py-2 px-2 text-cyan-400 font-semibold">To</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {flightPaths.map((path, index) => (
                      <tr key={path.id} className="hover:bg-white/5 transition-colors">
                        <td className="py-2 px-2 text-cyan-300">{path.start.name}</td>
                        <td className="py-2 px-2 text-blue-300">{path.end.name}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            return (
              <group key={path.id}>
                ...
                <FlightPath
                  ...
                  onProgressUpdate={(progress) => updateFlightPathProgress(path.id, progress)}
                />
              </group>
            )
          })}
...
function Earth({
  ...
  const oceanColor = `rgb(${Math.floor(oceanBrightness * 80)}, ${Math.floor(oceanBrightness * 160)}, ${Math.floor(oceanBrightness * 220)})`
  ...
  return (
    <mesh ref={meshRef} castShadow receiveShadow>
      ...
      <meshStandardMaterial
        ...
        emissive="#1a3a52"
        emissiveIntensity={landGlow * 1.3}
        color="#2d5a7e"
        roughness={0.85}
        metalness={0.05}
        toneMapped={false}
  ...
  return (
    <>
      <mesh scale={1.018}>
        ...
        <meshBasicMaterial color="#00d9ff" transparent opacity={0.2} toneMapped={false} side={2} />
      </mesh>
      <mesh scale={1.012}>
        <sphereGeometry args={[1, 64, 64]} />
        <meshBasicMaterial color="#0099ff" transparent opacity={0.15} toneMapped={false} side={2} />
      </mesh>
      <mesh scale={1.008}>
        <sphereGeometry args={[1, 64, 64]} />
        <meshBasicMaterial color="#1a6fa8" transparent opacity={0.1} toneMapped={false} side={2} />
      </mesh>
    </>
  )
}
